...
export default function Home() {
  ...
  // １分足解析を実行する関数
  const handleOneMinuteAnalysis = () => {
    if (!candlestickData || candlestickData.length === 0) {
      if (analysisTextareaRef.current) {
        analysisTextareaRef.current.value = '解析エラー: データが読み込まれていません。';
      }
      return;
    }

    // 陽線と陰線を分類
    const upCandles: number[] = []; // 陽線の長さ（close - open）
    const downCandles: number[] = []; // 陰線の長さ（open - close）

    for (const candle of candlestickData) {
      const length = Math.abs(candle.close - candle.open);
      
      if (candle.close > candle.open) {
        // 陽線（終値が始値より高い）
        upCandles.push(candle.close - candle.open);
      } else if (candle.close < candle.open) {
        // 陰線（終値が始値より低い）
        downCandles.push(candle.open - candle.close);
      }
      // ドージ（始値=終値）はスキップ
    }

    // 陽線の平均と最大を計算
    const upAverage = upCandles.length > 0
      ? upCandles.reduce((sum, len) => sum + len, 0) / upCandles.length
      : 0;
    const upMax = upCandles.length > 0
      ? Math.max(...upCandles)
      : 0;

    // 陰線の平均と最大を計算
    const downAverage = downCandles.length > 0
      ? downCandles.reduce((sum, len) => sum + len, 0) / downCandles.length
      : 0;
    const downMax = downCandles.length > 0
      ? Math.max(...downCandles)
      : 0;
    ...
    const result = [
      ...
      '',
      `【陽線の統計】`,
      `  平均の長さ: ${upAverage.toFixed(priceDecimalPlaces)}`,
      `  最大長さ: ${upMax.toFixed(priceDecimalPlaces)}`,
      `  陽線の数: ${upCandles.length}本`,
      '',
      `【陰線の統計】`,
      `  平均の長さ: ${downAverage.toFixed(priceDecimalPlaces)}`,
      `  最大長さ: ${downMax.toFixed(priceDecimalPlaces)}`,
      `  陰線の数: ${downCandles.length}本`,
      '',
      `【全体の統計】`,
      `  総ローソク足数: ${candlestickData.length}本`,
      `  陽線の割合: ${((upCandles.length / candlestickData.length) * 100).toFixed(1)}%`,
      `  陰線の割合: ${((downCandles.length / candlestickData.length) * 100).toFixed(1)}%`,
    ].join('\n');
